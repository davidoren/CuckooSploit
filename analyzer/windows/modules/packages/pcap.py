...
class Pcap(Package):
    ...
    def finish(self):
        ...
        '''
        try:
            log.info("killing tshark process")
            self.tshark_proc.kill()
            log.info("opening tshark pcap file and sending to result server")
            
            with open(self.tshark_filename, "rb") as f:
                log.info("opened tshark.pcap of {0} bytes".format(os.stat(self.tshark_filename).st_size))
                tshark_pcap = f.read()
                nf = NetlogFile("files/tshark.pcap")
                nf.sock.sendall(tshark_pcap)
                nf.close()
                log.info("sent tshark pcap to result server")
        except Exception, e:
            log.error("Error stopping packet capture: " + str(e))
        finally:
            return super(Pcap, self).finish()
        '''